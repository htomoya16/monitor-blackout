...
## 3. 必須要件（WHAT）
### 3.1 ユーザー操作
- ユーザーは黒幕を出したい画面をアクティブにし、`MonitorBlackout.Toggle.exe` を実行する  
- **ショートカット実行 → 即トグル（ON→OFF/ OFF→ON）**
- ON中：**オーバーレイを1回クリック → OFF**

### 3.2 対象モニタ判定
- 起動時はまずカーソル位置から対象モニタを判定する
- 取得失敗時はアクティブウィンドウ由来のモニタをフォールバックする
...
### 4.2 対象モニタ取得アルゴリズム
1) マウスカーソル位置を取得し、`MonitorFromPoint()` でモニタを判定する
2) 判定に失敗した場合、現在アクティブウィンドウから対象を取得する
3) 取得したモニタ領域を黒幕生成のバウンディングボックスにする
